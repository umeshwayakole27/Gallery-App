...
/**
 * Helper class for Google Lens integration, adapted from the Tulsi App.
 */
 ...
 *
 * @param uriString The URI string of the image to search with Google Lens
 ...
 */
fun searchWithGoogleLens(uriString: String, context: Context): Boolean {
    ...
    try {
        ...
        val contentUri = if (uri.scheme == "content") {
            ...
            try {
                ...
                FileProvider.getUriForFile(
                    context,
                    "com.uw.simplegallery.fileprovider",
                    file
                )
            } catch (e: Exception) {
        ...
        // If all local approaches failed, open web
        if (!success) {
            try {
